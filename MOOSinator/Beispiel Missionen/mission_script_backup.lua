...
-- 🛠️ Event Handling für Landung & Mechaniker-Spawning

-- Falls AirbaseMonitor schon existiert, zuerst alte Events abmelden
if AirbaseMonitor then
  ...
  function AirbaseMonitor:OnEventLand(EventData)
    if EventData.IniUnit and EventData.IniUnit:IsAlive() then
      ...
      elseif string.match(groupName, "^AirbasePatrol2$") or string.match(groupName, "^AirbasePatrol2#%d+$") then
          ...
          trigger.action.outText(" Tatsächlicher Gruppenname: " .. groupName, 15)
      end
    end
  end
    ...
    if group and group:IsAlive() then
        for _, unit in pairs(group:GetUnits()) do
            if unit:GetVelocityKMH() > 1 then
                -- Falls sich das Flugzeug noch bewegt, später erneut prüfen
                SCHEDULER:New(nil, function() CheckShutdown(groupName) end, {}, 5)
                return
            end
        end
  ...
  function SpawnMechanic(groupName)
      local spawnPos = nil
      local mechanicName = nil  -- Standardwert setzen
      local group = GROUP:FindByName(groupName)
                  ...
                  if group and group:IsAlive() then
                      ...
                      if unit then
                          ...
                          if destinationVec3 then
                              ...
                              local destinationY = destinationVec3.y  -- Korrekt für DCS!
                              local destinationZ = destinationVec3.z
                              local destination = POINT_VEC3:New(destinationX, destinationY, destinationZ)
  
                              trigger.action.outText("📍 Mechaniker fährt nach X: " .. destinationX .. ", Y: " .. destinationY .. ", Z: " .. destinationZ, 15)
                              
                              -- 📌 DCS bekommt eine direkte Fahr-Anweisung
                              local route = {
                                id = 'Mission',
                                params = {
                                    route = {
                                        points = {
                                            [1] = {
                                                x = destination.x,
                                                y = destination.z, -- In DCS wird y als "Höhe" genutzt, also nehmen wir Z!
                                                action = 'Cone',
                                                speed = 5, -- Geschwindigkeit in m/s
                                                formation = 'Column',
                                                task = {}
                                            }
                                        }
